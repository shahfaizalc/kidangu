...
class FragmentSavedStories : Fragment() {

    lateinit var adView: AdView


    @Transient
    var binding: ContentsavedstoriesBinding? = null;
    ...
    private fun bindView(inflater: LayoutInflater, container: ViewGroup?): View {
        if (binding == null) {
            binding = DataBindingUtil.inflate<ContentsavedstoriesBinding>(
                inflater,
                R.layout.contentsavedstories,
                container,
                false
            )
            val areaViewModel = SavedStoriesViewModel(requireActivity(), this)
